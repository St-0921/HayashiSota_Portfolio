...
### 3. Reconcile with UIChan（9人チーム制作 / リードプログラマ）
...
サブシーンはすべて、IInitializableインタフェースの実装、
開始イベントの購読、終了時のイベント発行という共通要件を満たします。
Unity標準のライフサイクルに任せて自動で初期化されるのを避け、
ManagerScene側から明示的に呼ぶ設計とすることで、
初期化のタイミングと順序を一元的に制御できるようにしました。

**シーン構成と全体の流れ**

![UIアクションゲーム シーン構成](ui_scene_flow.png)

サブシーンはすべて、IInitializableインタフェースの実装、開始イベントの
購読、終了時のイベント発行という共通要件を満たし、ManagerSceneから
統一的に扱える設計としました。

**1サイクルの動作（全サブシーン共通）**

![UIアクションゲーム シーン動作のシーケンス](ui_event_sequence.png)
